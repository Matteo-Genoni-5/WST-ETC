...
def get_vega_flux_zeropoints(bandm, quantity):
    if(bandm not in filters_vega):
        raise Exception("ERROR: Filter not present -> choose from [U,B,V,R,I,J,H,K]")

    if(quantity not in ["Fv", "Fll", "PHll"]):
        raise Exception("ERROR: Filter not present -> choose from [Fv, Fll, PHll]")
...
def vega_to_ab_converter(mag, bandm):
    ...
    if(bandm not in filters_AB):
        raise Exception("ERROR: Bandpass NOT present -> choose from: " + ', '.join(filters_AB))
...
def ab_to_vega_converter(mag, bandm):
    ...
    if(bandm not in filters_vega):
        raise Exception("ERROR: Bandpass NOT present -> choose from: " + ', '.join(filters_vega))

    return mag - ab_vega_diff_vf[filters_vega.index(bandm)]  # Mvega = Mab - MAG
